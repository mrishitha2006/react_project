...
const RCDetails = () => {
  ...
  // Find RC
  const rc = centersData.rcs.find((r) => r.id === parseInt(id));

  // Combine all projects
  const allProjects = useMemo(() => {
    if (!rc) return [];
    return [
      { ...rc.bestProject, category: "best", id: "best-project" },
      ...rc.ongoing.map((p, index) => ({
        ...p,
        category: "ongoing",
        id: `ongoing-${index}`,
      })),
      ...rc.completed.map((p, index) => ({
        ...p,
        category: "completed",
        id: `completed-${index}`,
      })),
  ...
  if (!rc) return <h2>RC not found</h2>;
  ...
  return (
    <div className="rc-details-container">
      <div className="rc-banner">
        <h1 className="rc-title">{rc.name}</h1>
      </div>
